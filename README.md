...
- **[beta]** [yantrikdb-mcp](https://github.com/yantrikos/yantrikdb-mcp) by [yantrikos](https://github.com/yantrikos) - Cognitive memory backend with contradiction detection, consolidation, and temporal decay. Unlike append-only vector stores, YantrikDB actively manages memory: `think()` consolidates duplicate memories, detects when stored facts conflict ("CEO is Alice" vs "CEO is Bob"), and fades unimportant memories via configurable half-life. Self-hosted Rust backend with HA cluster option.
