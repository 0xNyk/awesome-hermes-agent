...
- **[production]** [onequery-cli](https://github.com/wordbricks/skills/tree/main/skills/onequery-cli) by [Wordbricks](https://github.com/wordbricks) - Installable agent skill for safe, auditable read-only SQL queries against approved company data sources. Routes Hermes agents through centralized credentials, source capability checks, query validation, row limits, and audit logs instead of direct database credentials. Apache-2.0, tagged releases, actively maintained. Disclosure: author is affiliated with Wordbricks/OneQuery.
