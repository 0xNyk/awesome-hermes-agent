...
- **[experimental]** [knos-hermes](https://github.com/drexthealpha/knos-hermes) by [drexthealpha](https://github.com/drexthealpha) - Memory provider backed by [Knos](https://github.com/drexthealpha/Knos): one local SQLite memory that Hermes shares with Claude Code, Cursor and OpenCode on the same machine, so a decision recorded in one is visible to the others. Adds claim/withhold — work another agent claimed is refused, not just flagged. No server, no API key, no model download. `pip install knos-hermes && knos-hermes install`
