...
- **[beta]** [windy-access](https://github.com/sneakyfree/windy-connect) by [sneakyfree](https://github.com/sneakyfree) - One-command pairing for the Windy ecosystem. `windy connect` auto-detects your runtime and drops a marker-bounded block into `~/.hermes/.env` with native `EMAIL_*`/`IMAP_*`/`SMTP_*`/`WINDY_MIND_*` vars + auto-installs `SKILL.md` into `~/.hermes/skills/windy-access/`. Gives the agent a real `@windymail.ai` mailbox (Stalwart-backed), a Matrix chat identity, OpenAI-compatible LLM access (Windy Mind), and an optional Eternitas Passport for agent-to-agent trust. Magic-link email auth — no Google account required. `windy disconnect --yes` reverses everything cleanly. MIT-licensed, on PyPI + ClawHub.
