...
- **[beta]** [hermes-skills (jiawood2006)](https://github.com/jiawood2006/hermes-skills) by [jiawood2006](https://github.com/jiawood2006) - Three practical skills: video-to-text (Douyin/local video transcription), de-ai-writer (Chinese copy de-AI), and doc-ocr (PDF/scan OCR via macOS Vision). Standard SKILL.md format.
...
- **[beta]** [taskmarket-skill](https://github.com/arjunkshah12345-hash/hermes-skill-taskmarket) by [arjunkshah12345-hash](https://github.com/arjunkshah12345-hash) - Browse, create, submit, and review Taskmarket USDC tasks on Base. Paid writes require explicit approval. Install: `hermes skills install https://raw.githubusercontent.com/arjunkshah12345-hash/hermes-skill-taskmarket/main/SKILL.md --name taskmarket`.
- **[beta]** [Alter](https://github.com/alter-persona/alter) by [alter-persona](https://github.com/alter-persona) - Builds a digital persona of the user from their own words — interviews via chat/voice (local whisper.cpp transcription), creates a base persona at 30 spoken minutes, then improves from corrections, docs, and AI chat exports. Runtime generation is local-model-first (Ollama); corpus lives in local Postgres + pgvector. Optional voice replies via user's ElevenLabs clone. Eight sealed validation questions judge persona accuracy; contradictions trigger clarification. Apache-2.0.
- **[experimental]** [shopify-review-triage](https://github.com/alfredtech2026/shopify-app-review-brief/tree/main/docs/skills/shopify-review-triage) by [alfredtech2026](https://github.com/alfredtech2026) - Portable MIT-licensed skill that turns user-supplied public 1–3-star Shopify App Store review rows into an evidence-linked P0–P3 triage brief. Markdown-only workflow with no network, scripts, packages, credentials, or paid service required. Preserves source links and enforces public-data-only input. MIT.
- **[experimental]** [hermes-startup](https://github.com/33hodl/hermes-startup) by [33hodl](https://github.com/33hodl) - Guided first-dollar onboarding: 10-question personal profile, ranked evidence-labeled idea map, and one prepaid balance for per-call tool spend. Privacy-safe (answers stay local). Explicitly not an income guarantee. MIT.
- **[production]** [hermes-video-skills](https://github.com/pierrelucd670/hermes-video-skills) by [pierrelucd670](https://github.com/pierrelucd670) - Three production-ready video generation skills tested on A40 48GB: ltx23-generate (LTX-2.3 + synced audio), wan22-generate (Wan 2.2 Turbo 4-step with LightX2V LoRA), and pinkcherry-nsfw (PinkCherry v1.6 + Heretic Gemma3 uncensored). Documents 5 critical ComfyUI API workflow bugs and hardware-specific A40 optimizations. Install: `hermes skills tap add pierrelucd670/hermes-video-skills`.
...
- **[beta]** [gondola-provider](https://github.com/Gondola-Market/hermes-gondola-provider) by [Gondola-Market](https://github.com/Gondola-Market) - Adds Gondola as a native model provider: pay-as-you-go USDC-settled access to a marketplace on Venice AI supply over one OpenAI-compatible endpoint, no subscription. Filters mixed catalog to chat-capable models and adds `hermes doctor` connectivity row. Prompts and completions never persisted. 100+ models including GPT-5.6, Claude Opus 5, Gemini 3.6 Flash, Grok 4.20. MIT.
...
- **[beta]** [Hermes Connector](https://github.com/CorsenAI/hermes-connector) by [CorsenAI](https://github.com/CorsenAI) - MIT-licensed Chrome extension and local companion binding exact Hermes profile/session to user-selected signed-in Chrome tabs. Scoped read, navigation, click, typing, screenshot, tab-management through authenticated loopback broker. Available from Chrome Web Store; no relay or browsing telemetry. MIT.
...
- **[experimental]** [Agent Trust Kit](https://github.com/mauricemohr88-debug/agent-trust-kit) by [mauricemohr88-debug](https://github.com/mauricemohr88-debug) - Open-source plugin and standalone toolkit for bounded AI-agent handoffs. Creates explicit allowlist-based packets from clean Git revisions, requires exact return manifests and evidence receipts, quarantines returned files. Lets controller independently recheck evidence without executing worker commands or merging automatically. MIT.
...
- **[production]** [Hermes-OTG](https://github.com/MilkyWay008/Hermes-OTG) by [MilkyWay008](https://github.com/MilkyWay008) - Fully portable, zero-install Hermes Agent that runs from a USB stick or any drive/folder on Windows — no Python/Node/Docker on host, no admin rights, no registry writes. Runs side-by-side with installed Hermes desktop (different mutex, port 7642 vs 8642, isolated data). Source-patched portability survives drive-letter changes. Bundles Python 3.12, PortableGit, FastMCP/FastAPI/CLI-Anything, windows-mcp + 31 Sysinternals rescue tools, and full Hermes skill library. MIT, v1.0.0.
...
- **[beta]** [schedulala-agent](https://github.com/schedulala/schedulala-agent) by [schedulala](https://github.com/schedulala) - Agent skill + official MCP server for scheduling and publishing social posts to 12 platforms (Instagram, TikTok, X, LinkedIn, YouTube, Facebook, Threads, Bluesky, Pinterest, Mastodon, Telegram, Google Business). First comments through the API on 8 platforms via one field — thread-targeted on X/Threads/Bluesky, with per-platform delivery status reported back. Sandbox (sk_test_) keys simulate entire posting loop without publishing. Hosted remote MCP at `https://schedulala.com/api/mcp`, stdio via `npx -y @schedulala/mcp-server`, or CLI-only. MIT.
