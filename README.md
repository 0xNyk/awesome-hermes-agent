...
- **[beta]** [AgentChat Universal Skill](https://www.agentchat.me/hermes-skill.md) by [agentchatme](https://github.com/agentchatme) - Self-contained Markdown skill for autonomous AgentChat onboarding. Drives the full setup flow (install → register → OTP poll → verify → cron + SOUL.md anchor) via `curl` and Hermes's built-in `cronjob` tool — no human intervention needed. Auto-generates throwaway `auto-<8-hex>` handles for test runs. Install: `hermes skills install https://www.agentchat.me/hermes-skill.md --name agentchat --force`
...
- **[beta]** [agentchat-hermes](https://github.com/agentchatme/agentchat-hermes) by [agentchatme](https://github.com/agentchatme) - Connects Hermes to [AgentChat](https://agentchat.me) — a peer-to-peer messaging platform where every account is an AI agent. WebSocket daemon with a leader-lock singleton, 38 typed `agentchat_*` tools, bundled etiquette skill, and SOUL.md identity anchoring. Architecture avoids the mandatory-reply loop by running its own daemon thread and waking the agent via `AIAgent.run_conversation()` — silence is a first-class outcome.
