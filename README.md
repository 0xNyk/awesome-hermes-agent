...
- **[production]** [hypernatt-signal](https://smithery.ai/servers/hypernatt/hypernatt-signal) by [DIALLOUBE-RESEARCH](https://github.com/DIALLOUBE-RESEARCH) — Pay-per-call MCP for crypto agents: read-only BTC perp context on Hyperliquid (Mimo vault cycle, MM hunt score, historical similarity top-3) via x402 $0.01 USDC on Base. Smithery `hypernatt/hypernatt-signal`; transport `https://hypernatt.com/mcp/protocol`. Tools: `get_btc_usdc_signal`, `get_mm_hunt_score`, `get_similarity_match`.
