...
- **[beta]** [wikiskill](https://github.com/ashutoshsinghpr7/wikiskill) by [ashutoshsinghpr7](https://github.com/ashutoshsinghpr7) - Faithful implementation of Google's WikiSkill paper (arXiv 2608.27454): the agent evolves its own skills via raw→maintainer→proposer→gate with strict statistical gating (`R_val > R_best`, git rollback). Hermes is the reference backend; multi-agent (Claude Code in the box). Live gates have rejected harmful skills; ~$0.09/iteration on free-tier models. Ships a skills tap: `hermes skills tap add ashutoshsinghpr7/wikiskill`.
