...
- **[beta]** [hermes-agentmemory](https://github.com/MukundaKatta/hermes-agentmemory) by [MukundaKatta](https://github.com/MukundaKatta) - Pull-model episodic memory plugin that takes deletion seriously. No background work, every write is synchronous, and `agentmemory_forget` removes events with no tombstone. `$HERMES_HOME/agentmemory/trace.jsonl` is a tail-able audit of every prefetch. Three tools (`agentmemory_recall`, `agentmemory_forget`, `agentmemory_drift`), three lifecycle hooks, under 600 lines of Python. Trade-off: 200ms-2s cold-start tax in exchange for real deletes and full auditability. 7 smoke tests, green CI on Python 3.10-3.13 across Linux + macOS, MIT.
