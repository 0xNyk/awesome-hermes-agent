...
- **[beta]** [hermes-bus-plugin](https://github.com/mlinquan/hermes-bus-plugin) by [LinQuan](https://github.com/mlinquan) - Message bus integration plugin for Hermes Agent. Auto-starts bus daemon, registers session as bus endpoint, injects external bus messages into LLM context via `pre_llm_call` hook. Provides `bus_send`, `bus_status`, `bus_info` tools for cross-process communication between Hermes sessions, external agents, and notification daemons. Enables multi-agent coordination over Unix sockets.
- **[beta]** [hermes-snow-search](https://github.com/mlinquan/hermes-snow-search) by [LinQuan & Snow (AI Girl)](https://github.com/mlinquan) - In-memory parallel search plugin that loads sessions, holographic facts, and built-in memory into RAM at startup. Searches all three stores concurrently — <0.5ms latency vs ~350–700ms serial. Also on [PyPI](https://pypi.org/project/hermes-snow-search/).
...
- **[beta]** [hermes-bus](https://github.com/mlinquan/hermes-bus) by [LinQuan](https://github.com/mlinquan) - Generic Unix Socket message bus transport — 4-byte framing protocol, endpoint registration, heartbeat keep-alive, auto-reconnect, thread-safe message queue. Short-lived fire-and-forget + long-lived persistent connections. Configurable hook system for post-route callbacks. Zero dependencies.
- **[beta]** [hermes-notify](https://github.com/mlinquan/hermes-notify) by [LinQuan](https://github.com/mlinquan) - Config-driven notification router for Hermes Agent — YAML rule matching by message type, context injection into LLM, audio playback, command execution, tmux notification sender. Pairs with hermes-bus for cross-process agent communication.
