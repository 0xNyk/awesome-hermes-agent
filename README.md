...
- **[production]** [ponytail](https://github.com/dietrichgebert/ponytail) by [DietrichGebert](https://github.com/dietrichgebert) - Lazy senior-dev mode for coding agents: a "do only what the task needs" ladder (YAGNI → reuse → stdlib → native platform feature → installed dependency → one line → minimal code) that cuts ~54% of code while keeping every safety, validation, error-handling, and accessibility guard. Ships six skills (`ponytail`, `ponytail-review`, `ponytail-audit`, `ponytail-debt`, `ponytail-gain`, `ponytail-help`) with lite/full/ultra/off intensity levels. Native Hermes install via `hermes plugins install DietrichGebert/ponytail --enable`. Benchmarked on real agent sessions as the only arm that cut LOC, tokens, cost, and time while staying 100% safe. MIT.
...
- **[production]** [last30days](https://github.com/mvanhorn/last30days-skill) by [mvanhorn](https://github.com/mvanhorn) - AI agent-led search engine that searches *people*, not editors — researches any topic (person, company, product, or "X vs Y") across Reddit, X, YouTube, TikTok, Hacker News, Polymarket, GitHub, Bluesky, LinkedIn, and the web from the last 30 days, scored by real engagement and synthesized into one cited brief. Free sources (Reddit, HN, Polymarket, GitHub) work with zero config; X/YouTube/TikTok/others via your own keys or browser sessions. Agentskills.io-compatible including a first-class Hermes install (`hermes skills install mvanhorn/last30days-skill/skills/last30days --force`). MIT.
