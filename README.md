...
- **[beta]** [hermes-codex-skin](https://github.com/FPSUnleashed/hermes-codex-skin) by [FPSUnleashed](https://github.com/FPSUnleashed) - Independent Hermes Desktop visual plugin: Codex-inspired chat layout while preserving native behavior, themes, Glass, model and Thinking Level menus, voice, and task surfaces. Local Composer width and pinned-message settings. Does not modify Hermes source, add a backend, or make network requests. MIT.
...
- **[beta]** [hermes-artifact-relay](https://github.com/eloktev/hermes-artifact-relay) by [eloktev](https://github.com/eloktev) - Native plugin plus MIT [artifact-relay](https://github.com/eloktev/artifact-relay) service for private delivery of long Markdown and HTML. Pinned install (`--ref 262db8cc06a66fc2b95ee9f55dc32f1a6fb637d4 --no-enable`). Separate publish/view credentials; no CDN execution path. MIT.
...
- **[beta]** [hermes-openai-codex-web-search](https://github.com/bart6114/hermes-openai-codex-web-search) by [bart6114](https://github.com/bart6114) - OpenAI Codex hosted search as a Hermes web-search backend using an existing ChatGPT/Codex sign-in. Uses that subscription; no separate search API key. MIT.
...
- **[beta]** [hermes-hud](https://github.com/Diabloluo/hermes-hud) by [Diabloluo](https://github.com/Diabloluo) - User-level plugin for the official hermes dashboard: local-first observability (health, estimated/actual/unbilled cost, sessions, cron, channel jitter, incidents). Read-only core DBs, 127.0.0.1, no outbound telemetry. Distinct from [joeynyc/hermes-hud](https://github.com/joeynyc/hermes-hud) (a TUI). MIT.
...
- **[beta]** [plur-hermes](https://github.com/plur-ai/plur/tree/main/packages/hermes) by [plur-ai](https://github.com/plur-ai) - Native Hermes memory plugin for the PLUR engram store. Requires the `@plur-ai/cli` CLI (`npm i -g @plur-ai/cli`); then `pip install plur-hermes`, auto-discovered on gateway start with no extra config. Captures corrections and preferences as YAML engrams at `~/.plur/`. The store and all retrieval run on your machine — BM25 + local BGE-small embeddings, no external service for storage or search. Selected engrams are injected into the prompt you were already sending, so PLUR adds no network path of its own; optional `plur_sync` pushes your store to a git remote if you configure one. 22 tools including `plur_learn`, `plur_recall`, `plur_inject`, `plur_feedback`, `plur_forget`. Retrieval improves with feedback ratings; unused engrams decay via ACT-R. Zero Python runtime dependencies. Apache-2.0.
- **[beta]** [Mnemosyne](https://github.com/ElonAug7/Mnemosyne-agentmemory-engine-openclaw-hermes) by [ElonAug7](https://github.com/ElonAug7) - Zero-dependency, local-first cognitive memory engine for AI agents. Compound-cue retrieval powered by 31 cognitive-science papers: 10-factor scoring (importance, recency decay, primacy, retrieval-induced forgetting, testing effect) with true BM25 ranking plus a pure-local semantic dictionary (800+ synonyms, no LLM, no embedding API). Pure Markdown storage, 7-way parallel search (<50ms), built-in Web UI, native Hermes MemoryProvider bridge + skill, and an OpenClaw gateway hook. Beats every embedding system on the Memory-Native benchmark (nDCG@10 0.238, local-only systems #1). Windows/macOS/Linux, zero API keys. GPLv3.
...
- **[beta]** [causal-memory](https://github.com/JingxuanC/causal-memory/tree/main/hermes-plugin) by [JingxuanC](https://github.com/JingxuanC) - Memory provider with a causal core: flat facts plus typed decision→outcome edges (caused / enabled / prevented) on a per-profile local SQLite store. Wired hooks for system-prompt causal directory, budgeted prefetch recall, and non-blocking write-behind; the store lives outside the context window, so memory survives Hermes compaction. Rust core, Apache-2.0.
...
- **[beta]** [Hermes-OTG](https://github.com/MilkyWay008/Hermes-OTG) by [MilkyWay008](https://github.com/MilkyWay008) - Hermes on a USB stick — a rescue companion for recovering a broken host install without rebuilding the machine.
