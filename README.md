...
- **[beta]** [hermes-feishu-zh](https://github.com/OLDBAI213/hermes-feishu-zh) by [OLDBAI213](https://github.com/OLDBAI213) - Community extension for Windows Hermes users that enables Chinese Feishu display — replaces English UI labels with Chinese (审批、确认、拒绝 etc.), stabilizes Markdown output via `post` format, and optionally adds the `lark-cli-toolbox` plugin with 12 Feishu tools (docs, messages, tasks, calendar, base). One-click install with config merge, full backup/rollback/uninstall support, and CI tested. The only community extension targeting Chinese Feishu users.
