...
- **[beta]** [cronalytics](https://github.com/8bit64k/cronalytics) by [8bit64k](https://github.com/8bit64k) - Cron cost and usage analytics plugin for Hermes Agent. Hooks `on_session_end`, stores derived analytics in a local SQLite fact database, and surfaces them via a Hermes dashboard tab, a CLI tool, and an agent skill for cron health diagnostics and optimization recommendations. The only Hermes resource tracking per-cron-job token costs in detail. 80+ stars in first 3 weeks. Multi-language support (en, zh, zh-TW, es). v1.1.0.
