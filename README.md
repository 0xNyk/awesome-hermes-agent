...
- **[beta]** [1claw-hermes](https://github.com/1clawAI/1claw-hermes) by [1clawAI](https://github.com/1clawAI) - TypeScript integration bringing [1Claw](https://1claw.xyz) to Hermes Agent. Provides MCP-based secret fetching from an HSM-backed vault, a Shroud TEE LLM proxy sidecar (redacts secrets/PII and blocks prompt injection before prompts reach the model provider), per-subagent identities, and the Intents API for keyless on-chain signing. Hermes agents get just-in-time, scoped, revocable credentials without keys touching the agent context.
