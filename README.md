...
- **[beta]** [eagle-eye](https://github.com/willingning-coder/eagle-eye) by [willingning-coder](https://github.com/willingning-coder) - 5-layer intelligent skill routing engine for Hermes. Solves the skill overload problem when 50+ skills are installed by narrowing candidates to the top-5 most relevant before each API call. Pipeline: Hard Triggers (deterministic) → FTS5 BM25 → Synonym Dictionary → Dense Embedding → RRF Fusion. Confidence gate ensures "not matching" is a valid result — LLM retains final authority. Uses `pre_llm_call` hook with zero core modification. Includes auto-config generator and bilingual (EN/CN) LLM prompts.
