...
- **[beta]** [ui-design-agent-kit](https://github.com/muzimu217/ui-design-agent-kit) by [muzimu217](https://github.com/muzimu217) - End-to-end UI design workflow: researches real references, freezes a user-approved plan, enforces a design contract (semantic tokens, do/don't rules, spring motion presets), implements, then verifies the UI in a real browser with screenshot evidence. Ships 11 demo products (2 playable 3D demos), a 33-source inspiration library with license notes, and a cross-project taste profile. Host-agnostic: Codex-native bundle plus single-file prompt export (`npm run prompt:build:lean`) that works in Hermes via system-prompt injection.
