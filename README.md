...
- **[beta]** [memex-hermes](https://pypi.org/project/memex-hermes/) by [parallelclaw](https://github.com/parallelclaw) - Bridges Hermes into the [memex-mvp](https://www.npmjs.com/package/memex-mvp) unified-memory corpus — one searchable history across Hermes + Claude Code + OpenClaw + Telegram + Cursor, etc. Turns are stored verbatim in local SQLite + FTS5 (no extraction, no cloud, no auth). **Single-client Hermes users likely don't need this** — Hermes built-in memory already covers that case; this plugin earns its place when you use 2+ AI clients. Multi-path capture (sync_turn + state.db tailing + on_session_end safety net), one-shot `memex-hermes-backfill` for existing history, three MCP tools (`memex_search` / `memex_get` / `memex_recent`) with progressive disclosure. `pip install memex-hermes && memex-hermes init`. MIT, stdlib only. [Source](https://github.com/parallelclaw/memex-mvp/tree/main/plugins/memex-hermes).
