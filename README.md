...
- **[production]** [skillcraft](https://github.com/cloudroad-io/skillcraft) by [cloudroad-io](https://github.com/cloudroad-io) - Linter + sync CLI for AI agent-config files (`SKILL.md`, `CLAUDE.md`, `AGENTS.md`, `.cursor/rules`, `.claude/rules`, copilot-instructions). Pluggable rule set (SC1xx–SC4xx), one `AGENTS.md` canonical source generating and drift-checking every format, SARIF output for CI. Vendor-neutral — works with Hermes and any agent that reads these files. Python, `pip install skillcraft`, MIT.
