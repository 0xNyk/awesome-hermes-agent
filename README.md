...
- **[beta]** [deja-vu](https://github.com/vshulcz/deja-vu) by [vshulcz](https://github.com/vshulcz) - Recall from the other twenty coding agents on the machine (Claude Code, Codex, Cursor, OpenClaw): a `pre_llm_call` plugin over a local index of the transcripts they already wrote, plus a skill installable with `hermes skills install vshulcz/deja-vu/skills/deja-search`. No LLM, no network, nothing leaves the machine.
