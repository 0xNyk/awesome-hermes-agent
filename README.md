...
- **[beta]** [kesha-voice-kit](https://github.com/drakulavich/kesha-voice-kit) by [drakulavich](https://github.com/drakulavich) - Local-first CLI voice backend for Hermes Agent. Provides STT in 25 languages and TTS via local command providers with no API keys required. Plugs into Hermes's `HERMES_LOCAL_STT_COMMAND` and `tts.providers.<name>.type: command` surfaces — keeping voice input and spoken replies local instead of routing audio through a cloud API. Ships a [dedicated Hermes guide](https://github.com/drakulavich/kesha-voice-kit/blob/main/docs/hermes.md) with copy-paste config for both STT and TTS.
