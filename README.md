...
- **[experimental]** [hermes-drive-index](https://github.com/gregoryhorn/hermes-drive-index) by [gregoryhorn](https://github.com/gregoryhorn) - Private local Google Drive full-text search index for Hermes. Crawls selected Drive folders, extracts document text, stores chunks in a local SQLite FTS5 database, and exposes `drive_index_search`, `drive_index_status`, and `drive_index_update` Hermes tools. Keeps indexed text on the user's machine — fast, token-friendly, privacy-first. Apache-2.0.
