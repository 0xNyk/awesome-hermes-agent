...
- **[production]** [BuyWhere MCP Server](https://github.com/BuyWhere/buywhere) by [BuyWhere](https://github.com/BuyWhere) - Agent-native product catalog API for Southeast Asia commerce. Search 1.5M+ products across Shopee, Lazada, Amazon SG, Carousell, FairPrice, and 50+ merchants. Real-time price comparison, deal discovery, and multi-currency support (SGD, USD, MYR, IDR, THB, PHP, VND). 6 tools for product search, detail lookup, price comparison, deals, category browsing, and category products. Free tier available (1,000 calls/month). Wire it into Hermes via MCP configuration.
